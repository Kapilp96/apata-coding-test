...
class CreditCardManagementSystem {
    ...
    updateAmount(cardNumber, amount) {
        const encryptedCardNumber = (0, security_1.encrypt)(cardNumber);
        const creditCard = this.database.get(encryptedCardNumber);
        if (!creditCard) {
    ...
    deleteCreditCard(cardNumber) {
        const encryptedCardNumber = (0, security_1.encrypt)(cardNumber);
        return this.database.delete(encryptedCardNumber);
    }
    charge(cardNumber, amount) {
        const encryptedCardNumber = (0, security_1.encrypt)(cardNumber);
        console.log("charge: ", encryptedCardNumber, this.database.has(encryptedCardNumber));
        const creditCard = this.database.get(encryptedCardNumber);
    ...
    chargeAmount(cardNumber, amount) {
        const encryptedCardNumber = (0, security_1.encrypt)(cardNumber);
        console.log("charge: ", encryptedCardNumber, cardNumber);
        const creditCard = this.database.get(encryptedCardNumber);
        if (!creditCard) {
            return false;
        }
    ...
    creditAmount(type, cardNumber, amount) {
        const encryptedCardNumber = (0, security_1.encrypt)(cardNumber);
        const creditCard = this.database.get(encryptedCardNumber);
        if (!creditCard) {
            return false;
